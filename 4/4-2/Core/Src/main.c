...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
        HAL_UART_Receive_IT(&huart1, Uart1Temp, 1); // 开启串口接收中断
        HAL_UART_Transmit(&huart1, Uart1Temp, 1, 0xFFFF); // 发送接收到的数据
    }
}
/* USER CODE END 0 */
...
static void MX_USART1_UART_Init(void)
{
  ...
  HAL_UART_Receive_IT(&huart1, Uart1Temp, 1); // 开启串口1接收中断，接收数据存放在Uart1Temp数组中，长度为1字节
