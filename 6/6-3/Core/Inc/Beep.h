...
extern unsigned short BGM_Length; // 乐谱长度变量，存储当前乐谱的长度
extern Note_TypeDef * BGM_Current; // 当前乐谱指针，指向正在播放的乐谱数组
extern unsigned char BGM_Volume; // 乐谱音量变量，存储当前乐谱的音量等级
extern unsigned char BGM_ChangeFlag; // 乐谱切换标志, 当需要切换乐谱时设置为1，在定时器中断处理函数中检测并切换乐谱
extern const Note_TypeDef TwoTigersNote[]; // 《两只老虎》乐谱数组，包含音符频率和时值
extern const Note_TypeDef PeppaPigNote[]; // 《小猪佩奇》乐谱数组，包含音符频率和时值
extern const Note_TypeDef DouDiZhuNote[]; // 《斗地主》乐谱数组，包含音符频率和时值
...
void BGM_Play(unsigned char volume_level, Note_TypeDef * BGM); // 播放背景音乐函数，参数为音量等级和乐谱数组
